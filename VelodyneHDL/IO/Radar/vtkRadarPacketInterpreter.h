...
#ifndef VTKRADARPROVIDERINTERNAL_H
#define VTKRADARPROVIDERINTERNAL_H
...
class VTK_EXPORT  vtkRadarPacketInterpreter : public vtkAlgorithm
{
public:
  vtkTypeMacro(vtkRadarPacketInterpreter, vtkAlgorithm)
  ...
  /**
   * @brief IsRadarPacket check if the given packet is really a radar packet
   * @param data raw data packet
   * @param dataLength size of the data packet
   */
  virtual bool IsRadarPacket(unsigned char const * data, unsigned int dataLength) = 0;
  ...
  /**
   * @copydoc RadarPacketInterpreter::LaserSelection
   */
...
protected:
  ...
  //! depending on the radar. Note that it is note necessarily expressed in Hz, in case of
  //! rotational radar, this correspond to the RPM (Rotation Per Minute).
  double Frequency = 0;
  ...
  //! Indicate if the vtkRadarProvider::SensorTransform is apply
  bool ApplyTransform = false;

  //! Fixed transform to apply to the Radar points.
  ...
  //! - vtkRadarProvider::CropModeEnum::Cartesian it correspond to [X_min, X_max, Y_min, Y_max, Z_min, Z_max]
  //! - vtkRadarProvider::CropModeEnum::Spherical it correspond to [R_min, R_max, THETA_min, THETA_max, PHI_min, PHI_max]
  //! - vtkRadarProvider::CropModeEnum::Spherical -> Note implemented yet
  //! all distance are in cm and all angle are in degree
  double CropRegion[6] = {0,0,0,0,0,0};

  vtkRadarPacketInterpreter() { printf("vtkRadarPacketInterpreter\n"); };
  virtual ~vtkRadarPacketInterpreter() = default;

private:
  vtkRadarPacketInterpreter(const vtkRadarPacketInterpreter&) = delete;
  void operator=(const vtkRadarPacketInterpreter&) = delete;

};

#endif // VTKRADARPROVIDERINTERNAL_H
