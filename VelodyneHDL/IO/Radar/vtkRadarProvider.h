...
#ifndef VTKRADARPROVIDER_H
#define VTKRADARPROVIDER_H
...
/**
 * @brief The vtkRadarProvider class is a pure abstract class which provides a
 * common interface that all Radar subclasses should implement
 */
class VTK_EXPORT vtkRadarProvider : public vtkPolyDataAlgorithm
{
public:
  vtkTypeMacro(vtkRadarProvider, vtkPolyDataAlgorithm)
  ...
  /**
   * @copydoc vtkRadarPacketInterpreter::CalibrationFileName
   */
  ...
  /**
   * @copydoc vtkRadarPacketInterpreter
   */
  vtkGetObjectMacro(Interpreter, vtkRadarPacketInterpreter)
  virtual void SetInterpreter(vtkRadarPacketInterpreter *);

  vtkMTimeType GetMTime() override;

  friend class vtkRadarReaderInternal;
  friend class vtkRadarStreamInternal;
protected:
  vtkRadarProvider();
  ~vtkRadarProvider() = default;
  ...
  vtkRadarPacketInterpreter* Interpreter = nullptr;

  //! The calibrationFileName to used and set to the Interpreter once one has been set
  std::string CalibrationFileName = "";
private:
  vtkRadarProvider(const vtkRadarProvider&) = delete;
  void operator=(const vtkRadarProvider&) = delete;

};

#endif // VTKRADARPROVIDER_H
